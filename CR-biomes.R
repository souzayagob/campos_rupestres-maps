...
#Composing the map (testing)
cr_plot  <- spplot(campos_rup, zcol = "sector",
                   xlim = c(-50.5, -38.5), ylim = c(-23.25, -8.5), 
                   sp.layout = list(list(biomes, col = "transparent", 
                                         fill = c("#D2D2D2", "#A0A0A0", "#6E6E6E"))),
                   colorkey = FALSE,
                   scales = list(draw = FALSE),
                   col = campos_rup$color_in,
                   col.regions = makeTransparent(c("#0000A3", "#008805", "#FF7F00", "#FF0005"), alpha = 0.5)) 