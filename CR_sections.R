...
#Note that the Southern Espinha̤o and the Northern Espinha̤o are delimitedusing federative unities, while the Brazilian Central Plateau and the Serra da Canastra are delimited using the polygons generated above
...
for(i in 1:nrow(campos_rup@data)){
  if(campos_rup$sector[i] == "Southern Espinha̤o"){
    campos_rup$color_in[i] <- makeTransparent("#0000A3", alpha = 0.65)
  } else if(campos_rup$sector[i] == "Northern Espinha̤o"){
